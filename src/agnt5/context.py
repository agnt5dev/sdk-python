...
if TYPE_CHECKING:
    from .agent import StatefulAgentDefinition
    from .decorators import ComponentExecutionResult, StateTransitionPayload
...
class StepCheckpoint:
    ...
    def from_payload(cls, payload: Dict[str, Any]) -> "StepCheckpoint":
        ...
        if isinstance(timestamp_raw, str):
            ...
            updated_at = datetime.now(timezone.utc)
        return cls(name=name, key=key, status=status, attempt=attempt, result=result, updated_at=updated_at)
...
class StepCheckpointStore:
    ...
    def record_success(self, name: str, key: Optional[str], result: Any, attempt: int) -> StepCheckpoint:
...
class Context:
    ...
    def __init__(
        ...
        *,
        ...
        self.component_type = component_type or str(self._metadata.get("component_type", "function"))
        self.object_id = object_id or self._metadata.get("object_id")
        self.method_name = method_name or self._metadata.get("method_name")
        self.journal_position = journal_position if journal_position is not None else self._metadata.get("journal_position")
    ...
    def _parse_checkpoints(
        raw: Optional[Sequence[Dict[str, Any]]]
    ) -> List[StepCheckpoint]:
    ...
    async def sleep(self, seconds: float) -> None:
        ...
        if not self.run_id:
            self._logger.debug("No run_id available; falling back to asyncio.sleep for '%s'", timer_key)
        ...
        self._checkpoints.record_success("__sleep__", timer_key, {"seconds": seconds}, attempt=self.attempt)
    ...
    async def llm_complete(
        ...
        *,
        ...
        try:
            response = await client.complete(prompt=prompt, model=model, schema=schema, context=self)
            ...
            if response.usage.total_tokens:
                metrics.observe("llm.tokens.total", response.usage.total_tokens, model=model, mode="complete")
            return response
    ...
    async def llm_chat(
        ...
        *,
        ...
        try:
            ...
            if response.usage.total_tokens:
                metrics.observe("llm.tokens.total", response.usage.total_tokens, model=model, mode=mode)
            return response
        except LlmError as exc:
            duration_ms = (time.perf_counter() - start) * 1000.0
            metrics = self.metrics()
    ...
    def _execute_component_sync(
        ...
        from .decorators import execute_component  # Local import to avoid cycle
...
class MemoryClient:
    ...
    def _load_snapshot(self, snapshot: Optional[bytes]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("Failed to decode state snapshot for object %s: %s", self._object_id, exc)
        return {}
...
class AgentClient:
    ...
    @staticmethod
    def _get_registry():
        from .agent import get_agent_registry  # Local import to avoid cycle
    ...
    def _resolve_object_id(
        ...
        if not resolved:
            raise ValueError(
                f"Agent '{definition.name}' produced an empty object_id"
            )
...
class _GatewayTransport:
    ...
    def request(self, method: str, path: str, body: Optional[Mapping[str, Any]] = None) -> Mapping[str, Any]:
...
class ToolClient:
    ...
    def register(
        ...
        *,
        ...
        if exists:
            self._context.log().warning("Tool '%s' was re-registered; overriding previous handler", name)
        return tool
