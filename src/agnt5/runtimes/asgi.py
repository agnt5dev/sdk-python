...
from ..function import execute_component, get_registered_functions
...
class ASGIRuntime(RuntimeAdapter):
            ...
            if response.success:
                ...
                try:
                    ...
                    await self._send_json_response(
                        send, 200, {"result": output, "invocation_id": response.invocation_id}
                    )
                ...
                await self._send_error(
                    send, 500, response.error_message or "Function execution failed"
                )
    ...
    async def _handle_list_functions(self, scope: dict, receive, send):
        ...
        for name, func in functions.items():
            info = {"name": name, "type": "function"}
            if hasattr(func, "__doc__") and func.__doc__:
                info["description"] = func.__doc__.strip()
            function_info.append(info)

        await self._send_json_response(
            send, 200, {"functions": function_info, "count": len(function_info)}
        )
    ...
    async def _send_error(self, send, status: int, message: str):
        """Send error response."""
        await self._send_json_response(send, status, {"error": message, "status": status})
    ...
    async def handle_request(
        ...
        try:
            ...
            return InvocationResponse(
                invocation_id=request.invocation_id, output_data=result_data, success=True
            )
