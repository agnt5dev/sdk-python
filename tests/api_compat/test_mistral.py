"""
Mistral API Compatibility Tests

Tests real Mistral API connections to catch breaking changes.
Run weekly via CI to ensure SDK compatibility with Mistral APIs.

Models tested:
- mistral-small-latest (cost-effective - primary test model)
...
Run with:
    MISTRAL_API_KEY=... pytest tests/api_compat/test_mistral.py -v
"""
...
from .conftest import skip_without_mistral


# All tests in this module require Mistral API key
pytestmark = [
...
@pytest.mark.asyncio
@pytest.mark.skip(reason="Mistral Large may have higher rate limits or require billing - using Small for regular tests")
async def test_generate_mistral_large():
...
@pytest.mark.asyncio
@pytest.mark.skip(reason="Codestral may require separate API access")
async def test_generate_codestral():
    """Test with Codestral model (for code)."""
    response = await lm.generate(
    ...
    assert response is not None
    assert response.text is not None
    assert "def" in response.text.lower()
...
async def test_invalid_model_error():
    ...
    with pytest.raises(Exception):
        await lm.generate(
            model="mistral/not-a-real-model-12345",
            prompt="Hello",
        )
...
async def test_function_calling_basic():
    ...
    model = _LanguageModel(provider="mistral", default_model=None)
...
async def test_parallel_tool_calls(multi_tool_set, tool_executor):
    ...
    Mistral supports parallel tool calls via the OpenAI-compatible API.
    """
    model = _LanguageModel(provider="mistral", default_model=None)
