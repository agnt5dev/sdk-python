...
- Re-export `discover_skills` and `resolve_skills` from `agnt5` and
  ...
  re-exported them, so the documented `from agnt5 import discover_skills`
  raised `ImportError`. The sibling helper `discover_agents_md` was
  exported, as is the equivalent `discoverSkills` in the TypeScript SDK.
