...
use agnt5_sdk_core::pb::{RuntimeMessage, ServiceMessage, ComponentInfo, InvokeFunctionResponse, runtime_message};
...
impl PyWorkerConfig {
    ...
    pub fn new(service_name: String, service_version: String, service_type: String) -> Self {
        Self { service_name, service_version, service_type }
    }
}

impl From<PyWorkerConfig> for WorkerConfig {
    fn from(config: PyWorkerConfig) -> Self {
        WorkerConfig::new(config.service_name, config.service_version, config.service_type)
    }
}
...
impl PyWorker {
    ...
    fn set_message_handler(&self, handler: PyObject) -> PyResult<()> {
        ...
        let mut handler_guard = self.message_handler.lock()
            .map_err(|e| {
    ...
    fn initialize(&self) -> PyResult<()> {
        log::info!("Initializing PyWorker for service: {}", self.config.service_name);
        ...
        let worker = Worker::new(worker_config, components);
    ...
    fn run<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        log::info!("Starting PyWorker run for service: {}", self.config.service_name);
        ...
        future_into_py(py, async move {
            let worker = {
                let worker_guard = worker_arc.lock()
                    .map_err(|e| {
                        let err_msg = format!("Failed to lock worker: {}", e);
                        log::error!("{}", err_msg);
            ...
            let message_handler = move |runtime_message: RuntimeMessage| {
                ...
                async move {
                    Self::handle_runtime_message(handler_arc_inner, runtime_message).await
                }
            };
            ...
            match worker.run(message_handler).await {
                Ok(()) => {
                    log::info!("Worker event loop ended normally for service: {}", service_name);
                    Ok(())
                }
                Err(e) => {
...
impl PyWorker {
    ...
    async fn handle_runtime_message(
        ...
        log::debug!("Handling runtime message: {:?}", runtime_message.message_type);
        
        // Get the Python handler by cloning it properly
        let handler = {
            let handler_guard = handler_arc.lock()
                .map_err(|e| {
                    let err_msg = format!("Failed to lock message handler: {}", e);
                    log::error!("{}", err_msg);
            ...
            if let Some(handler) = handler_guard.as_ref() {
                ...
                return Err(agnt5_sdk_core::error::SdkError::Other(anyhow::anyhow!("No message handler set")));
            }
        };
        ...
        match runtime_message.message_data {
            Some(runtime_message::MessageData::InvokeFunction(invoke_request)) => {
                ...
                let result = Python::with_gil(|py| -> Result<Option<ServiceMessage>, agnt5_sdk_core::error::SdkError> {
            ...
            Some(runtime_message::MessageData::RegisterServiceResponse(_)) => {
                log::debug!("Ignoring RegisterServiceResponse message (type: {})", runtime_message.message_type as i32);
                Ok(None)
            }
            Some(runtime_message::MessageData::GetComponents(_)) => {
                log::debug!("Ignoring GetComponents message (type: {})", runtime_message.message_type as i32);
                Ok(None)
            }
            Some(runtime_message::MessageData::HealthResponse(_)) => {
                log::debug!("Ignoring HealthResponse message (type: {})", runtime_message.message_type as i32);
                Ok(None)
            }
            None => {
                log::debug!("Ignoring message with no data (type: {})", runtime_message.message_type as i32);
                Ok(None)
            }
        }
    }
